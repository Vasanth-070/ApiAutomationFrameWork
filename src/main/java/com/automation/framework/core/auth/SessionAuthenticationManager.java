...
public class SessionAuthenticationManager {
        ...
        if (instance == null) {
            ...
            try {
                if (instance == null) {
                    ...
                    logger.info("SessionAuthenticationManager instance created");
                }
            } finally {
                lock.unlock();
            }
    ...
    public String getSessionAuthToken(String sessionKey) {
        ...
        if (cachedAuth != null && !cachedAuth.isExpired()) {
            logger.debug("Using cached auth token for session: {}", sessionKey);
            return cachedAuth.getBearerToken();
        }
        
        // No cached token available, generate new one
        logger.info("Generating new authentication token for session: {}", sessionKey);
    ...
    private String generateAndCacheToken(String sessionKey) {
        ...
        try {
            ...
            SessionAuthData cachedAuth = sessionCache.get(sessionKey);
            if (cachedAuth != null && !cachedAuth.isExpired()) {
                logger.debug("Token was generated by another thread for session: {}", sessionKey);
                return cachedAuth.getBearerToken();
            }
            ...
            logger.info("Authenticating user: {} with client: {} for session: {}", loginId, clientId, sessionKey);
            ...
            if (response.isSuccess()) {
                ...
                logger.info("Authentication successful and cached for session: {}", sessionKey);
                
                return response.getBearerToken();
            } else {
                logger.error("Authentication failed for session: {} - {}", sessionKey, response.getMessage());
    ...
    public void clearSessionCache() {
        sessionCache.clear();
        logger.info("Session authentication cache cleared");
    }
    ...
    public void clearSession(String sessionKey) {
        ...
        if (removed != null) {
            logger.info("Cleared authentication for session: {}", sessionKey);
        }
    }
